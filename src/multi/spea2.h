	...
	for (int i=0; i < POPSIZE; i++) {
		for (int j=(i+1); j < POPSIZE; j++) {
			...
			if (MultiObjective::dominate (
				population[i]->obj,population[j]->obj,Info::Problem::get_instance()->objconf))
			{
				strenght[i]++;
			} else if (MultiObjective::dominate (
				population[j]->obj,population[i]->obj, Info::Problem::get_instance()->objconf)) 
			{
				strenght[j]++;
			}
...
void Spea2::densityCalc () {
	...
	for (int i = 0; i < POPSIZE; i++) {
		...
		for (int j = i; j < POPSIZE; j++) {
			...
			distance[i * POPSIZE + j].eDist = MultiObjective::distanceCalc (
				population[i]->obj, population[j]->obj, Info::Problem::get_instance()->OBJECTIVES);
...
void Spea2::printPop () {
	...
	for (int i=0; i < all_pop - ARCSIZE; i++) {
		...
		double obj1 = population[i]->ind.get_objective(0);
		double obj2 = population[i]->ind.get_objective(1);
		double obj3 = population[i]->ind.get_objective(2);
		printf ("%f %f %f\n",obj1, obj2, obj3);
	}
}
