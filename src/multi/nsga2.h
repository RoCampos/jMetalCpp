...
public:
	Nsga2 (int popsize = 10, int max_gen = 100,
			double p_cross = 0.5, double p_mut = 0.2);
	...
	for (int i = 0; i < (2 * m_popsize); ++i) {
		...
		for (int j = 0; j < i; ++j) {
			...
			if (MultiObjective::dominate(
					m_population[i]->obj,
					m_population[j]->obj, 
					Info::Problem::get_instance ()->objconf))
			{
...
void Nsga2::crownding_distance(int begin, int end) {
	...
	int OBJECTIVES = Info::Problem::get_instance ()->OBJECTIVES;
...
void Nsga2::initialization() {
	...
	m_population[0] = new GenericIndividual;
	m_population[0]->index = 0;
	for (int var = 1; var < m_popsize; ++var) {
...
void Nsga2::recombination() {
	...
	for (int i=0; i < m_popsize; i++) {
		...
		if ( p <= 0.25 ) {			
			crossover2 (p1->ind, p2->ind, ind->ind);
		} else if ( p >= 0.25 && p < 0.5) {
		 	crossover4 (p1->ind, p2->ind, ind->ind);
		} else if ( p >= 0.5 && p < 0.75) {
			crossover1 (p1->ind, p2->ind, ind->ind);
		} else {
			crossover3 (p1->ind, p2->ind, ind->ind);	
		}
...
void Nsga2::printPop() {
	...
	for (int i=0; i < (2 * m_popsize); i++) {
		...
		double obj1 = m_population[i]->ind.get_objective(0);
		double obj2 = m_population[i]->ind.get_objective(1);
		double obj3 = m_population[i]->ind.get_objective(2);
		printf ("%f %f %f\n",obj1, obj2, obj3);
	}
...
void Nsga2::printPopAsPisa () {
	...
	for (int i=0; i < (m_popsize); i++) {
		...
		if ((int)m_population[i]->fitness) {
			double obj1 = m_population[i]->ind.get_objective(0);
			double obj2 = m_population[i]->ind.get_objective(1);
			double obj3 = m_population[i]->ind.get_objective(2);
			if (obj1 > 0.0) printf ("%f %f %f\n",obj1, obj2, obj3);	
		}		
	}
	printf ("\n");
}
